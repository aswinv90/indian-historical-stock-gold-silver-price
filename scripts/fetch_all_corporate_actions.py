"""
fetch_all_corporate_actions.py
------------------------------
Bootstrap script to fetch complete historical Corporate Actions (1990 -> Present
plus upcoming announced events) directly from official exchange regulatory disclosures.

Captures:
- Bonus Issues (with ratio e.g. 1:1, 2:1)
- Stock Splits / Face Value Sub-Divisions (with split ratio e.g. 10:2, 10:1)
- Dividend Payouts (Interim, Special, Final with ₹/share amount)
- Rights Issues (with entitlement ratio)
- Buybacks
- Demergers & Amalgamations (Schemes of Arrangement)
- Capital Reductions & AGM/EGM dates
"""
...
def fetch_all_corporate_actions():
    ...
    # Defined historical date ranges
    windows = [
        ...
        ("01-01-2023", today_str),
        (today_str, future_date),  # Upcoming forward announcements
    ]

    all_raw = []
    log.info("Fetching complete corporate actions across all historical windows...")

    for start_w, end_w in windows:
        log.info(f"Querying window: {start_w} to {end_w}...")
        records = fetch_nse_chunk(start_w, end_w)
        log.info(f"Retrieved {len(records):,} records for {start_w} to {end_w}")
        all_raw.extend(records)
        time.sleep(1.0)  # Gentle delay between chunk requests

    if not all_raw:
        log.error("No corporate actions could be fetched.")
        return

    log.info(f"Total raw corporate action events collected: {len(all_raw):,}")
    ...
    df.drop_duplicates(subset=["Symbol", "Ex_Date", "Action_Type", "Purpose"], inplace=True)

    # Sort: records with Ex_Date descending (upcoming/latest first), then None at bottom
    df.sort_values(by=["Ex_Date", "Symbol"], ascending=[False, True], na_position="last", inplace=True)
    ...
    log.info(f"Saved {len(df):,} unique corporate action records to:")
    log.info(f" - Parquet: {PARQUET_FILE}")
    log.info(f" - CSV:     {CSV_FILE}")

    # Summary by Action_Type
    summary = df["Action_Type"].value_counts().to_dict()
    log.info(f"Corporate Actions Breakdown: {summary}")
